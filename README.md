...
*   **堅牢・こだわりポイント**:
    ...
    *   **スタンプとテキスト処理のバックエンド統合**: フロントエンドから送信されたスタンプは、バックエンド（Go）側で事前に紐づけられたテキストプロンプト（例：『この話題をもっと深掘りしてください』など）を持った通常の発言としてパースされます。これにより、テキスト入力とスタンプ送信の両方を**単一のAI処理フローとして完全に統一**し、堅牢なアーキテクチャを実現しています。
...
品質を担保し、デグレードを防ぐために多角的なテストアプローチを採用しています。

### **① Unit / Integration Test (Vitest)**
- **インタラクションの検証**: `React Testing Library` を用い、ユーザーの入力やボタンの活性状態、バリデーションロジックなどのコンポーネント単位の振る舞いを厳密にテストしています。
- **実行方法**: `npm run test` (frontend)

### **② Property-Based Testing (PBT)**
- **想定外の入力に対する堅牢性**: fast-check を導入。網羅的なランダム入力（境界条件や特殊文字など）を自動生成してコンポーネントに流し込み、どのような条件下でもクラッシュせずに正常なフォールバックが行われるかを検証します。
- **発見された実績**: `AgentIcon` において、`toString` などのプロトタイプ継承に起因する特殊な文字列が渡された際のクラッシュを PBT が特定し、未然に修正。ヒューマンエラーでは見落としがちなエッジケースを 100% カバーしています。
- **実行方法**: `npm run test:pbt`

### **③ Gherkin スタイルによる振る舞い検証 (BDD/E2E)**
- **シナリオベースの品質保証**: ユーザーの利用シーンを `Given / When / Then` 形式で定義した Gherkin スタイルのシナリオに基づき、システム全体の挙動を保証します。
- **Playwright による E2E**: 実際にブラウザ（Chromium）を起動し、以下の一連のコアフローが正常に動作することを自動検証します。
    - 認証バイパス設定を用いたログイン・ホーム表示
    - トークルーム作成と AI エージェントの自律対話
    - メッセージのリアルタイム受信とお気に入り登録
    - アイデアマップ上での物理操作とリサイクルボックスへの反映
- **実行方法**:
    1. 別ターミナルでバックエンドを起動: `cd backend && go run cmd/api/main.go`
    2. frontend ディレクトリで実行: `npm run test:e2e`
